...
def test_with_timeout(filename, inp, out, image, check_function):
...
def test_without_timeout(filename, inp, out, image, check_function):
    programa = [executable, conf.VALIDADOR_FUNCIONES, filename] if check_function else [executable, filename]
    try:
    ...
    for element in conf.work:
        ...
        for pos in range(len(tests)):
            ...
            with codecs.open(".fin", "w", "utf8") as in_file: 
                in_file.write(inp)
            isOk = do_test(filename, inp, out, image, pos==0 and conf.VALIDADOR_FUNCIONES!="")
            if not isOk:
                break  
        if isOk:
